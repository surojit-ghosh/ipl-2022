...
function VenueCard({ venue }: { venue: VenueSummary }) {
  return (
    <Link
      ...
      className="group rounded-lg border border-border bg-card px-5 py-5 transition-[background-color,border-color,transform] duration-[120ms] ease-[var(--ease-out)] hover:-translate-y-px hover:border-border-strong hover:bg-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:outline-none"
    >
  ...
  return (
    <div className="space-y-8">
      ...
      {venues.length ? (
        ...
        <EmptyState title="No venues available" description="The venue directory returned no IPL 2022 host stadium records." />
      )}
    </div>
  );
}
